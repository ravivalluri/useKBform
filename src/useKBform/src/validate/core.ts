import { IError, IInputElement } from '../models';
...
Validate.prototype.validate = function (): IError {
    ...
    if (arr.length) {
        ...
        this.hasAttribute('_password')?.forEach(({ value }: IInputElement): void => {
            if (!utils.isEmpty(value)) {
                this.hasAttribute('_strongpassword')?.forEach(({ value, name }) => {
        ...
        this.hasAttribute('_passwordrepeat')?.forEach(({ name, value }: IInputElement): void => {
            if (!utils.isEmpty(password)) {
                !utils.isEmpty(value) ? (passwordRepeat = value) && (finalName = name) : (errors[name] = 'please provide password repeat');
            }

            password !== passwordRepeat ? (errors[finalName] = 'passwords does not match') : (errors[finalName] = '');
        });
    }
