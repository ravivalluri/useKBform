...
export default function useKBform(): IUseKBform {
  ...
  const filterRefsExcept = useCallback(
    (refs: any, nodeNameExcept: string) => [...refs].filter(({ nodeName }: any) => nodeName !== nodeNameExcept),
    []
  );
  ...
  useEffect(() => {
    if (envMode === 'dev') {
      current.forEach(({ elements }) =>
        [...elements].forEach(el => (el.onkeyup = () => setWatchState(createSortedFormObjects())))
      );
    }
  }, [envMode]);
    ...
    current.forEach(({ attributes, elements }) => {
      if (attributes._formname.value === formName) {
        [...elements].forEach(el => {
          ...
          if (el.attributes._resetbtn && JSON.parse(el.value)) {
            el.click();
          }
        });
      }
