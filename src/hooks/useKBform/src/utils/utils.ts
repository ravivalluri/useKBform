import { IHTMLInputEvent } from '../types/types';
...
Utils.prototype.isNumberEvent = function (event: IHTMLInputEvent): boolean {
    ...
    if (((charCode as unknown) as number) > 31 && (((charCode as unknown) as number) < 48 || ((charCode as unknown) as number) > 57)) {
        return false;
    }
    return true;
};
