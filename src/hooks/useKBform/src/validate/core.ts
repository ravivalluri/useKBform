...
export default function Validate(this: any, array: Form[]) {
  this.array = array;
}
...
Validate.prototype.hasAttribute = function(attr: string) {
  return this.array?.filter((x: Form) => x.attributes.hasOwnProperty(attr));
};
...
Validate.prototype.validate = function() {
  ...
  this.hasAttribute('_required')?.forEach(({ name, value }: Form) => {
    if (utils.isEmpty(value)) errors[name] = 'this field is required';
    else errors[name] = '';
  });

  this.hasAttribute('_number')?.forEach(({ name, value }: Form) => {
  ...
  this.hasAttribute('_email')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_min')?.forEach(({ name, value, attributes }: Form) => {
    if (!utils.isEmpty(value)) {
      if (((value as unknown) as number) < parseInt(attributes._min.value))
        errors[name] = `min ${attributes._min.value} required`;
      else errors[name] = '';
    }
  });

  this.hasAttribute('_max')?.forEach(({ name, value, attributes }: Form) => {
    if (!utils.isEmpty(value)) {
      if (((value as unknown) as number) > parseInt(attributes._max.value)) errors[name] = `max ${attributes._max.value} allowed`;
      else errors[name] = '';
    }
  });

  this.hasAttribute('_minlength')?.forEach(({ name, value, attributes }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_maxlength')?.forEach(({ name, value, attributes }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_length')?.forEach(({ name, value, attributes }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_pin')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_amount')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_pan')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_panbasic')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_phone')?.forEach(({ name, value }: Form) => {
    if (!utils.isEmpty(value)) {
  ...
  if (arr.length) {
    ...
    this.hasAttribute('_password')?.forEach(({ value, name }: Form) => {
      if (!utils.isEmpty(value)) {
    ...
    this.hasAttribute('_passwordrepeat')?.forEach(({ name, value }: Form) => {
...
Validate.prototype.checkAttrCombinations = function() {
  this.array?.forEach(({ attributes }: Form) => {
