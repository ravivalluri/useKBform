...
export default function Validate(this: any, array: IForm[]): void {
  this.array = array;
}
...
Validate.prototype.hasAttribute = function(attr: string): void {
  return this.array[0]?.filter((x: IForm) => x.attributes.hasOwnProperty(attr));
};

/* method for validating form */
Validate.prototype.validate = function(): IForm {
  const errors = {} as IForm;

  this.hasAttribute('_required')?.forEach(({ name, value }: IForm): void => {
  ...
  this.hasAttribute('_number')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_email')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_min')?.forEach(({ name, value, attributes }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_max')?.forEach(({ name, value, attributes }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_minlength')?.forEach(({ name, value, attributes }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_maxlength')?.forEach(({ name, value, attributes }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_length')?.forEach(({ name, value, attributes }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_pin')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_amount')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_pan')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_panbasic')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  this.hasAttribute('_phone')?.forEach(({ name, value }: IForm): void => {
    if (!utils.isEmpty(value)) {
  ...
  //     this.hasAttribute('_password')?.forEach(({ value, name }: IForm): void => {
  //       if (!utils.isEmpty(value)) {
  ...
  //     this.hasAttribute('_passwordrepeat')?.forEach(({ name, value }: IForm): void => {
...
Validate.prototype.checkAttrCombinations = function(): void {
  this.array?.forEach(({ attributes }: IForm) => {
